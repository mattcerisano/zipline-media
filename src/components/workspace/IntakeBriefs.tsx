            ...
            {filteredIntakes.map(item => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 border-b border-white/5 pb-4">
                  <div>
                    <span className="px-2.5 py-0.5 rounded bg-accent/10 border border-accent/20 text-[9px] font-black uppercase tracking-widest text-accent">
                      {item.client_name}
                    </span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <select
                      ...
                      className={`text-[8px] font-black uppercase tracking-widest px-2.5 py-1.5 rounded-lg border cursor-pointer outline-none ${
                ...
                <div className="space-y-4 flex-grow">
                  {/* Creative brief text */}
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-wider text-white/30 flex items-center gap-1.5">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {item.aspect_ratios.map(ratio => (
                          <span key={ratio} className="px-2 py-0.5 bg-white/5 border border-white/5 rounded text-[9px] font-black tracking-wide text-white/80">
                            {ratio}
                          </span>
                        ))}
                      </div>
                        ...
                        {item.reference_links.length > 0 ? (
                          item.reference_links.map((url, i) => (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer"
                              className="text-[9px] font-semibold text-accent hover:underline truncate block max-w-[180px]"
                            >
                              {url.replace(/^https?:\/\/(www\.)?/i, '')}
                            </a>
                          ))
                        ) : (
                          <span className="text-[9px] text-white/20 italic">No links added</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="border-t border-white/5 pt-4 flex justify-end">
                  <button
                    ...
                    className="p-2 bg-red-500/5 hover:bg-red-500/15 border border-red-500/10 rounded-xl text-red-400/60 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all cursor-pointer flex items-center gap-1 text-[9px] font-black uppercase tracking-widest"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {refLinks.map((link, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded bg-white/5 border border-white/5 text-[9px] font-bold text-accent flex items-center gap-1.5">
